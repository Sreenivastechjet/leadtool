...
function DealDetails() {
  ...
  const getDetails = async (id) => {
    let res = await axios
      .get(`/api/v1/deal/getsingledeal/${id}`)
      .then((res) => {
        if (res) {
    ...
    if (note) {
      try {
        const res = await axios.patch(
          `/api/v1/deal/updateLeadnotes/${params.id}`,
          { newnote: note }
        );
        if (res) {
  ...
  const handleImgChange = async (e) => {
    ...
    if (selectedFile) {
      ...
      try {
        await axios.patch(
          `/api/v1/deal/updateLeadfile/${params.id}`,
          formData,
          {
            headers: {
                ...
                <div>
                  <div className="ms-3">
                    <div className="d-flex p-3 ps-0">
                      <h6> Business Name : <span className="fs-5">{deal.businessdetails}</span></h6>
                    </div>
                    {/* <div class="mb-3 row">
