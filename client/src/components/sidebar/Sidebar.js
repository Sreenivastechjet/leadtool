...
const Sidebar = () => {
  ...
  const [userRole] = context.authApi.userRole
  const sidebarData = SidebarData(userRole);
  ...
  const logoutUser = async () => {
      ...
      await axios.get("/api/v1/auth/logout").then((res)=>{
       if(res){
        localStorage.clear();
        toast.success("loggetout successfully")
        navigate("/login");
